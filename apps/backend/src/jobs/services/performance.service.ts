...
export class PerformanceService extends BaseJobService {
  ...
  protected async processOneUser(user: UserBatch): Promise<void> {
    await this.updateUserPerformance(user.userId);
  }

  public async updateUserPerformance(userId: string) {
    const cacheKey = `performance_${userId}`;

    const performanceData = await this.getCachedData(cacheKey, async () => {
      const [
        attemptsData,
