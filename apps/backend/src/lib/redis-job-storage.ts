...
class RedisJobStorage {
  ...
  async updateFileStatus(
    jobId: string,
    fileId: string,
    status: Partial<FileProcessingStatus>
  ): Promise<void> {
    try {
      const job = await this.get(jobId);
      if (!job) {
      ...
      if (status.status && oldStatus !== status.status) {
        switch (status.status) {
          ...
          case "failed":
            ...
            if (status.error) {
              job.errors.push(
                `${job.files[fileIndex].fileName}: ${status.error}`
              );
            }
            break;
          case "processing":
      ...
      console.error(
        `Failed to update file status for job ${jobId}, file ${fileId}:`,
        error
      );
      throw error;
    }
  ...
  async getJobWithMetadata(jobId: string): Promise<
    | (BulkUploadJob & {
        isExpired: boolean;
  ...
  async getNextJobFromQueue(): Promise<{
    jobId: string;
    priority: number;
    timestamp: number;
  } | null> {
