...
${subject.name === "Physics"
  ...
  `
      : subject.name === "Chemistry"
        ? `
      INSTRUCTIONS: 
      [NOTE]: USE MINIMUM THEORETICAL EXPLANATION AND ONLY SHOW IMPORTANT ONCES.
      ...
      - **Shortcut/Trick:** Mention an exam-oriented shortcut, pattern recognition, or heuristic to reach the answer faster. (If Possible)
...
RETURN EXACTLY THIS JSON (no extra text and each field need to follow strict rules like RENDER & FORMAT RULES):
{
  ...
  "format": "SINGLE_SELECT" | "MULTIPLE_SELECT" | "TRUE_FALSE" | "MATCHING" | "ASSERTION_REASON" | "COMPREHENSION" (e.g consider Statement-base -> "TRUE_FALSE", Match The Following -> "MATCHING", TYPE => IINTEGER then "SINGLE_SELECT"),
  "difficulty": 1|2|3|4,
  ...
  "pyqYear": "[Exam Name] Year (e.g., [JEE Main] 2024), " " if not mentioned",
