...
export class ReviewScheduleService {
  ...
  async processAllUsers() {
    ...
    while (true) {
      const userCount = await prisma.topicMastery.count();
  ...
  public calculateNextReview(params: ReviewParams): {
    nextReviewAt: Date;
    ...
    const reviewsBonus =
      completedReviews > 0 ? Math.log10(completedReviews) * 0.3 : 0;
  ...
  public calculateRetentionStrength(
  ...
  public calculateForgettingProbability(
    retentionStrength: number,
