        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 ">
            ...
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-3xl font-bold text-primary-900">{overview?.overallMastery?.percentage}%</span>
              <Badge variant='outline' className='bg-primary-500/10 text-primary-900'>
                {overview?.overallMastery?.label}
              </Badge>
            </div>
            <div className="mt-4">
              <Progress
                value={overview?.overallMastery?.percentage}
            ...
            <div className="md:mt-4 mt-2 text-sm flex items-center gap-2 text-primary-800">
              {(() => {
                const improvement = overview?.overallMastery?.improvement;
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <Card className=" border border-primary-100 p-4 ">
                ...
                <div className="mt-2 text-lg font-semibold flex items-center gap-2 ">
                  <span>{overview?.overallMastery?.label}</span>
                  {/* <span>{getMasteryEmoji(totalMastery)}</span> */}
                </div>
                <div className="mt-2 text-sm opacity-80 text-gray-800">
                  Top {overview?.overallMastery?.topPercentage}% of students
                </div>
              </Card>
