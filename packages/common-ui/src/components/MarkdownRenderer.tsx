...
const imageFloatClass = (loc: string) =>
  loc === "float-left" ? "float-left mr-3 mb-2" : loc === "float-right" ? "float-right ml-3 mb-2" : "";
  ...
  ({ content, className }) => {
    ...
    const components = useMemo(() => {
      return {
        ...
        h1: ({ children }: any) => <h1 className="text-xl font-bold text-gray-900  leading-relaxed tracking-wide">{children}</h1>,
        h2: ({ children }: any) => <h2 className="text-lg font-semibold text-gray-800  leading-relaxed tracking-wide">{children}</h2>,
        h3: ({ children }: any) => <h3 className="text-base font-medium text-gray-700  leading-relaxed tracking-wide">{children}</h3>,
