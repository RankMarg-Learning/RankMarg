...
class RedisClient {
  ...
  private async initializeClient() {
    try {
      const redisUrl = process.env.REDIS_URL_FRONTEND || process.env.UPSTASH_REDIS_REST_URL;
      
      if (!redisUrl) {
        console.warn('Redis URL not configured. Job storage will not persist across server restarts.');
        return;
      }
